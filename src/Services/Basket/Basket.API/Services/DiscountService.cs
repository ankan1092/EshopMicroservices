    ...
    namespace Basket.API.Services
    {
        public class DiscountService : IDiscountService
        {
            ...
            public async Task<DiscountResult> GetDiscountAsync(string productName, CancellationToken cancellationToken)
            {
                ...
                {
                    _logger.LogError(ex, "Circuit breaker is open for product: {ProductName}", productName);
                    return new DiscountResult(0, false, "Discount service temporarily unavailable: Circuit breaker is open");
                }
